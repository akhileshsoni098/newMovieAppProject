...
const saveMovie = async function (req, res) {
  ...
  if(!title){return res.status(400).send({status:false , message:" movie title is required "})}

if(typeof title != "string"){return res.status(400).send({status:false , message:" movie title should be in string "})}
title = dataB.title = title.trim()
if(title == ""){return res.status(400).send({status:false , message:"Please provide movie title"})}
...
// console.log(data)
   if(!data.Title){return res.status(400).send({status:false , message:"Please Provide valid title ...!"})}

let isExistMovie = await movieModel.findOne({title:data.Title, isDeleted:false})
// console.log(isExistMovie)
if(isExistMovie){return res.status(400).send({status:false , message:" This movie is already exist"})}
...
if(Object.keys(data).length == 0){
  return res.status(400).send({status:false , message:"Provide Input to search Movies"})
}
...
if((title && year)){
  ...
  if(!data.title){return res.status(400).send({status:false , message:" movie title is required "})}

  if(typeof title != "string"){return res.status(400).send({status:false , message:" movie title should be in string "})}
  title = data.title = title.trim()
  if(title == ""){return res.status(400).send({status:false , message:"Please provide movie title"})}
  
  if(!data.year){return res.status(400).send({status:false , message:" movie year is required "})}

  if(typeof year != "string"){return res.status(400).send({status:false , message:" movie year should be in string "})}
  year = data.year = year.trim()
  if(year == ""){return res.status(400).send({status:false , message:"Please provide movie year"})}
  
  const Query = await movieModel.find({title:title, year:year,  isDeleted:false })

if(Query.length == 0){return res.status(404).send({status:false , message:"no movie found by this filter"})}
...
try{
  ...
  if (!movieId) {
    return res.status(401).send({ status: false, message: " Provide valid movie " });
  }
...
if(!authorization){return res.status(404).send({status:false , message:"movie not found"})}
