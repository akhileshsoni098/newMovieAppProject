...
const checkMovie = await movieModel.findOne({_id:movieId, isDeleted:false})
if(!checkMovie){return res.status(400).send({status:false , message:"This movie does not exist"})}
...
let check = await watchListModel.findOne({movies:movieId, userID:userId})
if(!check){return res.status(404).send({status:false , message:" this movie is not present in your watchList "})}
